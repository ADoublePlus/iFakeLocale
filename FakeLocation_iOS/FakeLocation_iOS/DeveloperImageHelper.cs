        ...
        public static Tuple<string, string>[] GetLinksForDevice(DeviceInformation device)
        {
            ...
            if (VersionToImageUrlZip.Count == 0)
            {
                ...
                try
                {
                    ...
                    var response = WebClient.DownloadString("https://github.com/haikieu/xcode-developer-disk-image-all-platforms/tree-list/" + treeList);
                    var paths = response.Split('"')
            ...
            if (VersionToImageUrlOverride.Count == 0)
            {
                try
                {
                    ...
                    var response = JObject.Parse(WebClient.DownloadString("https://raw.githubusercontent.com/ADoublePlus/iFakeLocale/FakeLocation_iOS/updates.json"));
